...
export async function registerUser(data: Omit<User, 'id' | 'sessionId'>): Promise<User> {
  const usersSnap = await db.ref('users').get();
  const users: Record<string, User> = usersSnap.val() || {};

  for (const id in users) {
    if ((data.email && users[id].email === data.email) || (data.phone && users[id].phone === data.phone)) {
...
export async function loginUser(identifier: { email?: string; phone?: string }, password: string): Promise<User> {
  const usersSnap = await db.ref('users').get();
  const users: Record<string, User> = usersSnap.val() || {};
  ...
  for (const id in users) {
    if ((identifier.email && users[id].email === identifier.email) || (identifier.phone && users[id].phone === identifier.phone)) {
      foundId = id;
      foundUser = users[id];
      break;
    }
